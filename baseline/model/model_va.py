...
class BaselineModelVA(nn.Module):
    def __init__(self, pretrained=True, seq_len=32):
        super(BaselineModelVA, self).__init__()
        self.input_size = 512
        ...
        self.fc2 = nn.Linear(self.fc_size, self.fc_size2)
        self.fc_output_va = nn.Sequential(
            nn.Linear(self.fc_size2, self.output_dim),
            nn.Tanh()  # Add Tanh activation to limit output to (-1, 1)
        )
        self.fc_output_ar = nn.Sequential(
            nn.Linear(self.fc_size2, self.output_dim),
            nn.Tanh()  # Add Tanh activation to limit output to (-1, 1)
        )
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        x = self.fc2(x)
        v = self.fc_output_va(x)
        a = self.fc_output_ar(x)
        return (v, a)
...
if __name__ == "__main__":
    ...
    model = BaselineModelVA(pretrained=True, seq_len=32)
